    ...
    <div className="min-h-screen bg-[#050505] text-zinc-300 p-6 md:p-12 font-sans selection:bg-amber-500/30">
      <div className="max-w-7xl mx-auto mb-10 flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="flex items-center gap-6">
          <button onClick={() => router.push('/admin')} className="p-3 bg-white/5 border border-white/10 rounded-full hover:bg-white/10 transition-all group">
        ...
        <div className="relative w-full md:w-64">
          <select value={selectedEventId} onChange={(e) => setSelectedEventId(e.target.value)} className="appearance-none w-full bg-black border border-white/10 rounded-xl pl-6 pr-12 py-3 text-white outline-none focus:border-amber-500 font-medium cursor-pointer shadow-lg transition-all">
            {events.map(e => <option key={e.eventId} value={e.eventId} className="bg-neutral-900 text-white">{e.mainTitle}</option>)}
          </select>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        <GlassCard className="col-span-1 p-6 h-fit border border-white/5 bg-white/[0.02]">
          ...
          <form onSubmit={handleAddTable} className="space-y-4">
            <input type="text" required value={tableName} onChange={e => setTableName(e.target.value)} className="w-full bg-black border border-white/10 rounded-xl px-4 py-4 text-white outline-none focus:border-amber-500" placeholder="e.g. VIP-1" />
            <div className="grid grid-cols-2 gap-4">
              <input type="number" required value={capacity} onChange={e => setCapacity(e.target.value)} className="w-full bg-black border border-white/10 rounded-xl px-4 py-4 text-white outline-none focus:border-amber-500" placeholder="Pax" />
              <input type="number" required value={minSpend} onChange={e => setMinSpend(e.target.value)} className="w-full bg-black border border-white/10 rounded-xl px-4 py-4 text-white outline-none focus:border-amber-500" placeholder="Spend" />
            </div>
            <button type="submit" disabled={adding} className="w-full bg-amber-500 text-black font-bold uppercase tracking-widest py-4 rounded-xl hover:bg-amber-600 transition-all">
              {adding ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : "Deploy Table"}
            </button>
          </form>
        </GlassCard>

...
function TableBookingContent() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const resTables = await fetch(`/api/admin/tables?eventId=${eventId}&t=${Date.now()}`, { cache: 'no-store' });
        const resultTables = await resTables.json();
        if (resultTables.success) {
          setTables(resultTables.data.filter((t: any) => t.status === "Available"));
        }
        ...
        if (resultSettings.success && Array.isArray(resultSettings.data)) {
          const currentEvent: any = resultSettings.data.find((e: any) => e.eventId === eventId);
  ...
  const proceedToCheckout = () => {
    ...
    // 🚀 THE FIX: Corrected newSearchParams to new URLSearchParams
